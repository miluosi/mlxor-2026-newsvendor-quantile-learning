"""RSETO-IPA within-replication sample-size sensitivity on synthetic data."""
...
def build_parser():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=M_LIST,
        help="Initial m0 values; each m_k is samples generated per IPA replication.",
    )
    parser.add_argument(
        "--fixed-replications",
        type=int,
        default=16,
        help="Fixed independent IPA replications R while varying initial m0.",
    )
    parser.add_argument(
        "--fixed-lambda",
        type=float,
        default=0.5,
        help="Fixed NLL weight while varying initial m0.",
    )
    return parser
...
def main():
    ...
    if args.single_run:
        run_sensitivity(args, "m", args.m_list)
    else:
