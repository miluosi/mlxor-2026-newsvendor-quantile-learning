"""RSETO-IPA replication-count sensitivity on synthetic data."""
...
SIMULATION_NUM_LIST = [1, 4, 16, 64, 256]
DEFAULT_DIMS = [4, 9, 14, 19, 24]
...
def run_default_grid(args):
    ...
    for job_index, (dim, fold) in enumerate(jobs, start=1):
        ...
        print(
            f"\n[job {job_index}/{len(jobs)}] sweep=simulation_num "
...
def build_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        default=SIMULATION_NUM_LIST,
        help="Independent IPA replications R for each context.",
    )
    parser.add_argument(
        "--fixed-lambda",
        type=float,
        default=0.5,
        help="Fixed NLL weight while varying the replication count.",
    )
    return parser
...
def main():
    ...
    if args.single_run:
        run_sensitivity(args, "simulation_num", args.simulation_num_list)
    else:
